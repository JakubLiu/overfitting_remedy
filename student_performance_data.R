...
data <- fread('/media/DANE/home/jliu/MASTER_THESIS/Student_Performance.csv')

#exclude the binary variable 'Activities' (for simplicity)
data[,3] <- NULL
...
colnames(data_train)[length(colnames(data_train))] <- "y"
data_test <- data.frame(cbind(X_test, Y_test))
colnames(data_test)[length(colnames(data_test))] <- "y"


model_orig <- lm(y ~ ., data = data_train)
